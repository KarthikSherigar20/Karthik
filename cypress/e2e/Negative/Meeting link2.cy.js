...
describe('Meeting link-invalid doc& pat number',()=>{
    before(() => {
        cy.wrap(null).then(() => {
          login1();
        }).then(() => {
...
it('Meeting link-invalid doc& pat number',()=>{
...
cy.wait(2000);
cy.get('input[class="doctorConsult_input__iF2im"]').eq(0).type('948596857')
cy.wait(2000);
cy.get('input[class="doctorConsult_input__iF2im"]').eq(1).type('948254785')
cy.wait(2000);
