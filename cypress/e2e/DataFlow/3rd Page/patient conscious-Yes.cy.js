...
describe('Assement results-patient conscious-Yes.cy', () => {
    before(() => {
        cy.wrap((null)).then(() => {
            login()
        }).then(() => {
    ...
    it('Assement results-patient conscious-Yes.cy', () => {
        cy.get('body').then(($bodyText) => {
            ...
            if (bodyText.includes('New')) {
                ...
                cy.wait(1500);
                // cy.get('input[value="No"]').eq(2).click();
                // cy.wait(1500);
                ...
                cy.wait(1500);
                cy.get('p[class="text horizontalList_ans__3hClf"]').eq(2).invoke('text').then((text) => {
