...
describe('Assement results-Ambulance-YES.cy', () => {
    before(() => {
        cy.wrap((null)).then(() => {
            login()
        }).then(() => {
    ...
    it('Assement results-Ambulance-YES.cy', () => {
        cy.get('body').then(($bodyText) => {
            ...
            if (bodyText.includes('New')) {
                ...
                cy.wait(1500);
                cy.get('p[class="text horizontalList_ans__3hClf"]').eq(1).invoke('text').then((text) => {
